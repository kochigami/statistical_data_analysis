...
class UnpairedMultipleSampleTestOfNominalScale:
    def test(self, data):
        ...
        """
        sum_vertical = []
        sum_horizontal = [0 for i in range(len(data))]
        
        for i in range(len(data)):
            sum_vertical.append(sum(data[i]))
            for j in range(len(data[i])):
                sum_horizontal[j] += data[i][j]
        N = sum(sum_vertical)
        ...
        if 1 == 0:
            ...
            for i in range(len(data)):
                for j in range(len(data[i])):
                    chi_squared += (data[i][j] * data[i][j]) / float (sum_vertical[i] * sum_horizontal[j])  
            chi_squared -= 1.0
            chi_squared *= N
            df = (len(sum_vertical) - 1.0) * (len(sum_horizontal) - 1.0)
